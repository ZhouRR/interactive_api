(function () {
  ...
  var doneIconHtml = "<i class='dh-icon dh-icon-dice'>💾<svg><use xlink:href='#dh-done'/></svg></i>"
  ...
  var initDom = function(dom){
    var svgIcons = $("\
      <svg xmlns='http://www.w3.org/2000/svg' style='display: none;'>\
        ...
        <symbol id='dh-done' viewBox='-4 -4 32 32'>\
          <path fill='currentColor' fill-rule='nonzero' d='M19 11h-14v-2h14v2zm0 2h-14v2h14v-2zm0 4h-14v2h14v-2zm3-11v16h-20v-16h20zm2-6h-24v24h24v-24z'/>\
        </symbol>\
      </svg>\
    ");
    ...
    var btn = $("\
      <div class='actions'>\
        <a class='button primary' id='dh-history-done'>" + doneIconHtml + "</a>\
    ...
    var history = $("\
      <div class='dh-modal' id='dh-lottery-history'>\
        ...
        <div class='dh-number dh-modal-btn' style='font-size: xx-large;font-weight: bold;'>1</div>\
    ...
    $('#dh-history-done').click(function() {
      if(settings.done != undefined){
        settings.done(settings.winnerHistory);
      }
    });
  ...
  var initSelector = function() {
    ...
    if(settings.width != undefined) setSelectorSize(settings.width);
    setTimeout(function() {
...
//    console.log('Lottery: ' + settings.data.length + ' player');
    ...
    if(settings.width != undefined) setAvatarSize(settings.width);
  }

  // 设置头像大小
  var setAvatarSize = function(itemSideSize){
    $(".dh-lottery .avatar .image").css('height',itemSideSize+'px');
    $(".dh-lottery .avatar .image").css('width',itemSideSize+'px');
  }

  // 设置选择框大小
  var setSelectorSize = function(itemSideSize){
  ...
  var pushWinner = function(winnerProfile){
    ...
    new MaterialAvatar(el[0].getElementsByClassName('dh-name-avatar'), avatarOptions);
  }

  var moveToTarget = function(i,target) {
    $(profileEls[target]).addClass('current');
    if (!positionList[target]) return;
    move('#dh-lottery-selector #selector_'+i).x(positionList[target].left - 4).y(positionList[target].top - 4).ease('in-out').duration(settings.speed/2).end();
  ...
  var lotteryOnce = function(selector = 0){
    ...
    try {
        winnerProfile = JSON.parse(decodeURIComponent($($('.profile')[targetIndex]).data('profile')));
    } catch(err) {
        return false;
    }

    var invalidIds = ["2005001", "2005002", "2005005", "2005007", "2005028", "2006009"];
    // 已经在抽中,重新抽取
    if((settings.once && settings.winnerList[targetIndex]) || $.inArray(targetIndex,currentTarget)>=0 || $.inArray(winnerProfile.data.staff, invalidIds)>=0){
...
//      console.log('Lottery: moveToTarget #', currentTarget);
  ...
  var timing = function(timestamp, log) {
    if(typeof(log) != undefined) console.log(log);
    console.log($.now()-timestamp + ' ms');
    timestamp = $.now();
  }
  ...
  var showHistory = function(){
    ...
    new MaterialAvatar(box[0].getElementsByClassName('dh-name-avatar'), avatarOptions);
  ...
  var controller = {
    ...
    init : function (options) {
      defaultOptions.winners = [];
      defaultOptions.winnerList = [];
      defaultOptions.winnerHistory = [];
