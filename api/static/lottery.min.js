...
//    console.log('Lottery: ' + settings.data.length + ' player');
  ...
  var lotteryOnce = function(selector = 0){
    ...
    var winnerProfile = JSON.parse(decodeURIComponent($($('.profile')[targetIndex]).data('profile')));
    // 已经在抽中,重新抽取
    if((settings.once && settings.winnerList[targetIndex]) || $.inArray(targetIndex,currentTarget)>=0 || winnerProfile.data.staff == "2005028"){
